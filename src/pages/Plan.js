...
const Plan = () => {
    ...
    const monto1 = (i) => {
        if (monto) {
            setUser({...user, monto : user.monto + data[i].monto})
            setMonto(!monto)
        } else {
            setUser({...user, monto : user.monto - data[i].monto})
            setMonto(!monto)
        ...
        <div className='flex border-t border-gray5 xs:flex-col sm:flex-row'>
            ...
            <section className='flex xs:flex-col sm:flex-row xs:w-full sm:w-3/4 xs:px-0 sm:px-24 xs:pt-0 sm:pt-12 xs:pb-3 pb-16 sm:space-x-24'>
                ...
                <div className='xs:w-full sm:w-1/2'>
                    <div className='xs:hidden sm:block flex items-center'>
                    ...
                    <div className='text-gray2-p xs:pt-3 sm:mt-3 xs:bg-gray7 xs:px-8'>Conoce las coberturas para tu plan</div>
                    <div className='xs:bg-gray7 xs:pt-11 xs:pb-14 sm:mt-11 xs:px-8 '>
                    ...
                    <div className='flex xs:flex-col sm:flex-row xs:mx-8 justify-between mt-16'>
                        <div>
                            <div>Indica la suma asegurada</div>
                            <div className='flex text-xs text-gray2-p'>
                                <div className='pr-2'>MIN $12,500</div>
                                <div className='pl-2 border-l xs:mt-1'>MAX $16,500</div>
                            </div>
                        </div>
                    ...
                    <div className='text-xl mt-14 xs:mx-8'>Agrega o quita coberturas</div>
                ...
                {/* right */}
                <div className='xs:w-full sm:w-56 xs:flex xs:justify-between px-8'>
                    <div className='flex flex-col xs:justify-end xs:pb-0 sm:pb-5 border-b border-gray5'>
                        <div className='uppercase text-xs text-gray1-t mb-2 xs:hidden'>Monto</div>
                        <div className='text-3xl xs:text-2xl'>${user.monto}</div>
                        <div className='text-xs text-gray2-p'>mensuales</div>
                    </div>
                    <div className='mt-6 text-gray1-t xs:hidden'>El precio incluye:</div>
                    <div className='mt-3 text-gray2-p text-sm xs:hidden'>
